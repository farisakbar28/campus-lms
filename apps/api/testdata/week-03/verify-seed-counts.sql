...
WITH seed_tenants AS (
    ...
    SELECT
        ...
        (SELECT count(*) FROM course_staff cs JOIN seed_tenants st ON st.id = cs.tenant_id WHERE cs.source = 'seed') AS course_staff,
        ...
        (SELECT count(*) FROM course_staff cs JOIN course_offerings o ON o.id = cs.course_offering_id JOIN seed_tenants st ON st.id = cs.tenant_id WHERE cs.source = 'seed' AND cs.tenant_id <> o.tenant_id) AS course_staff_offering_mismatch,
        (SELECT count(*) FROM course_staff cs JOIN seed_tenants st ON st.id = cs.tenant_id LEFT JOIN memberships m ON m.tenant_id = cs.tenant_id AND m.user_id = cs.user_id WHERE cs.source = 'seed' AND m.id IS NULL) AS missing_lecturer_membership,
        (SELECT count(*) FROM course_staff cs JOIN seed_tenants st ON st.id = cs.tenant_id JOIN memberships m ON m.tenant_id = cs.tenant_id AND m.user_id = cs.user_id WHERE cs.source = 'seed' AND m.status <> 'active') AS inactive_lecturer_membership,
        (SELECT count(*) FROM course_staff cs JOIN seed_tenants st ON st.id = cs.tenant_id JOIN memberships m ON m.tenant_id = cs.tenant_id AND m.user_id = cs.user_id LEFT JOIN membership_roles mr ON mr.membership_id = m.id AND mr.role = 'lecturer' AND mr.revoked_at IS NULL WHERE cs.source = 'seed' AND mr.id IS NULL) AS missing_active_lecturer_role
)
